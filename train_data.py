...
# split train/test
df_train = df[df['starttime'] < '2017-04-01']
print(len(df_train))
df_test = df[df['starttime'] >= '2017-04-01']
print(len(df_test))
...
def compile_set(df, days):
    ...
    for i in range(len(days) - 1):
        ...
        df_day = df_day[['volume', 'percent spread', 'norm close']].stack().reset_index()
        ...
        df_day['next day norm close'] = (df[df['starttime'] == next_day]['close'].iloc[0] - p0) / p0
        df_day.index = [days[i]]
        sum_df = sum_df.append(df_day)
    return sum_df

# create training data
days = pd.date_range(start=df['starttime'].dt.date.min(), end='2017-04-01')
train = compile_set(df, days)
train.to_pickle('train.pkl')

# create test data
days = pd.date_range(start='2017-04-01', end=df['starttime'].dt.date.max())
