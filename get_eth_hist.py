...
dates = pd.date_range(start='2016-06-01', end='2017-07-01')
hist_cols = [ 'starttime', 'low', 'high', 'open', 'close', 'volume' ]

public_client = PublicClient()
rates_list = []
for i in range(len(dates) - 1):
    # gets 10 min candles per day
    rates_list.extend(public_client.get_product_historic_rates('ETH-USD', start=dates[i], end=dates[i+1], granularity=600))
...
eth_hist.to_pickle('eth_hist.pkl')
